...
async def __async_get_feed_entries(feed: FeedParserDict,
    ...
    for entry in feed.entries:
        ...
        description = await parse_description(description)

        location = getattr(entry, rss_config.location_tag) if hasattr(entry, rss_config.location_tag) else "NaN"

        location = await find_remote_pattern(location, description)
...
async def find_remote_pattern(location: str, description: str):
    """
    Finds the pattern 'remote' or 'anywhere' (case-insensitive) in the given text.
    
    Args:
        location (str): The text to search for the pattern.
        description (str): The text to search for the pattern.
        
    Returns:
        str: The matching word if found, otherwise "NaN".
    """
    pattern = r'\b(remote|anywhere)\b'
    match_location = re.search(pattern, location, re.IGNORECASE)
...
async def parse_description(description: str, parser: str = 'html.parser') -> str:
    """
    Parses the description to extract the first 100 characters.
    ...
    if parser != 'html.parser':
        raise AssertionError("The parser must be 'html.parser'")

    soup = BeautifulSoup(description, parser)
    ...
    for col in df.columns:
        if col == 'description':
            ...
            if not df[col].empty:  # Check if the column has any rows
                df[col] = df[col].astype(str)  # Convert the entire column to string
                df[col] = df[col].str.replace(r'<.*?>|[{}[\]\'",]', '', regex=True) #Remove html tags & other characters
                df[col] = df[col].str.replace(r'[%\d]', '', regex=True)  # Remove % and numbers
...
async def async_rss_reader(
    ...
    try:
        # NOTE: The fetch function is not used as the RSS feed is fetched directly from the URL
